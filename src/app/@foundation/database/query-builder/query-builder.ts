...
import { FindOperator } from '../find-options';
import { In } from '../find-options/operator';
import { EntityColumnNotFound } from '../error';
...
export abstract class QueryBuilder<Entity> {
   ...
   */
  update<T>(entity: QbjectType<T>, updateSet?: QueryDeepPartialEntity<T>): UpdateQueryBuilder<T>;
   ...
   * Sets entity's relation with which this query builder gonna work.
   */
  relation<T>(entityTarget: QbjectType<T> | string, propertyPath: string): RelationQueryBuilder<T>;
   ...
   */
  hasRelation<T>(target: QbjectType<T> | string, relation: string): boolean;
   ...
   * todo: move this method to manager? or create a shortcut?
   */
  hasRelation<T>(target: QbjectType<T> | string, relation: string | string[]): boolean {
