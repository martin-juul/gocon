import { RawSqlResultsToEntityTransformer } from './transformer';
...
import { RelationIdAttribute, RelationIdLoader, RelationIdMetadataToAttributeTransformer } from './relation-id';
import {
...
import { AbstractSqliteDriver } from '../driver/sqlite-abstract';
import { QueryResultCacheOptions } from '../cache/query-result-cache-options';
import { BroadcasterResult } from '../subscriber';
import { SelectQueryBuilderOption } from './select-query-builder-option';
...
export class SelectQueryBuilder<Entity> extends QueryBuilder<Entity> implements WhereExpression {
   ...
   * Removes all previously set from-s.
   */
  from<T>(entityTarget: QbjectType<T> | string, aliasName: string): SelectQueryBuilder<T>;
   ...
   * Removes all previously set from-s.
   */
  from<T>(entityTarget: QbjectType<T> | string | ((qb: SelectQueryBuilder<any>) => SelectQueryBuilder<any>), aliasName: string): SelectQueryBuilder<T> {
   ...
   * Also sets a main string alias of the selection data.
   */
  addFrom<T>(entityTarget: QbjectType<T> | string, aliasName: string): SelectQueryBuilder<T>;

  /**
   * Specifies FROM which entity's table select/update/delete will be executed.
   * Also sets a main string alias of the selection data.
   */
  addFrom<T>(entityTarget: QbjectType<T> | string | ((qb: SelectQueryBuilder<any>) => SelectQueryBuilder<any>), aliasName: string): SelectQueryBuilder<T> {
  ...
  protected createSelectDistinctExpression(): string {
    const {selectDistinct} = this.expressionMap;
      ...
      return ' ORDER BY ' + Object.keys(orderBys)
        .map(columnName => {
          if (typeof orderBys[columnName] === 'string') {
            ...
            return this.replacePropertyNames(columnName) + ' ' + (orderBys[columnName] as any).order + ' '
