...
import { RemoveOptions, SaveOptions } from '../repository';
import { BroadcasterResult } from '../subscriber';
import { ClosureSubjectExecutor, MaterializedPathSubjectExecutor, NestedSetSubjectExecutor } from './tree';
  ...
  protected async executeInsertOperations(): Promise<void> {
    ...
    await PromiseUtils.runInSequence(groupedInsertSubjectKeys, async groupName => {
      ...
      subjects.forEach(subject => {
        ...
        if (subject.changeMaps.length === 0 || subject.metadata.treeType) {
